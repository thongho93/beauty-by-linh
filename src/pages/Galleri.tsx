    ...
    <main className="relative min-h-screen overflow-hidden bg-[#070708] text-white">
      ...
      {lightbox && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Forstørret galleri"
        >
          <button
            type="button"
            aria-label="Lukk"
            className="absolute right-5 top-5 grid h-11 w-11 place-items-center rounded-full border border-[color:var(--color-gold)]/40 bg-black/30 text-[color:var(--color-gold)] transition-colors duration-300 hover:border-[color:var(--color-gold)] hover:bg-[color:var(--color-gold)]/10"
            onClick={() => setLightbox(null)}
          >
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 to-transparent px-5 py-4">
              <p className="text-[0.66rem] tracking-[0.24em] text-white/85">
                {lightbox.alt.toUpperCase()}
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="relative px-4 pb-10 pt-32 text-center">
        ...
        <h1
          className="text-4xl font-light tracking-[0.12em] text-white text-wrap-balance sm:text-5xl md:text-6xl"
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4">
            {galleryPhotos.map((photo) => (
              ...
              >
                ...
                <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(183,132,113,0.25),transparent_48%)] opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                <div className="absolute inset-x-0 bottom-0 px-3 py-2.5">
                  <p className="text-[0.58rem] tracking-[0.22em] text-white/82 sm:text-[0.62rem]">
          ...
          <div className="mt-12 text-center sm:mt-14">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center rounded-bl-xl rounded-br-none rounded-tl-xl rounded-tr-xl border border-[color:var(--color-gold)]/85 px-10 py-3 text-xs tracking-[0.3em] text-[color:var(--color-gold)] transition-colors duration-300 hover:bg-[color:var(--color-gold)]/12"
            >
