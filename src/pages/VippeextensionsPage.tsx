...
const faqs = [
  ...
  {
    q: "Skadet extensions mine naturlige vipper?",
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 mx-auto w-full max-w-5xl px-8 pb-14 pt-32 lg:px-16">
          ...
          <p className="mt-4 max-w-xl text-base text-white/60 leading-relaxed">
            Få fyldige, vakre vipper skreddersydd etter dine ønsker — fra naturlig volumøkning til dramatisk effekt.
          </p>
        </div>
      </section>
        ...
        <div className="grid gap-12 md:grid-cols-2 md:items-center">
          <div>
            ...
            <p className="mt-5 text-base leading-relaxed text-white/60">
              Vippeextensions er syntetiske eller silkefibre som festes enkeltvis på dine naturlige vipper med et
              medisinsk godkjent lim. Resultatet er lengre, tykkere og mer kurrente vipper som varer i uker —
              uten bruk av maskara.
            </p>
            <p className="mt-4 text-base leading-relaxed text-white/60">
              Vi tilpasser lengde, krumming, tykkelse og stil etter ditt ansikt og dine ønsker, slik at du får
              et resultat som føles naturlig og ser luksøst ut.
            </p>
            <button
              type="button"
              onClick={() => setBookingOpen(true)}
              className="mt-8 inline-flex items-center gap-3 rounded-tl-lg rounded-bl-lg rounded-br-lg rounded-tr-none border border-[color:var(--color-gold)] px-6 py-3.5 text-sm tracking-[0.25em] text-[color:var(--color-gold)] transition-all duration-300 hover:bg-[color:var(--color-gold)]/10 hover:shadow-[0_0_20px_rgba(183,132,113,0.25)] active:scale-[0.98]"
            >
              Bestill time
            </button>
          </div>
          <div className="overflow-hidden rounded-tr-2xl rounded-bl-2xl">
            <img
        ...
        <div className="mx-auto max-w-5xl px-8 py-16 lg:px-16">
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2">
            {types.map((t) => (
              <div
                key={t.title}
                className="rounded-tr-2xl rounded-bl-2xl border border-white/8 bg-[#111] p-6 transition-all duration-300 hover:border-[color:var(--color-gold)]/30 hover:shadow-[0_0_20px_rgba(183,132,113,0.08)]"
              >
                <h3
      ...
      <section className="mx-auto max-w-5xl px-8 py-16 lg:px-16">
        ...
        <ol className="mt-8 space-y-6">
          {[
            { step: "01", title: "Konsultasjon", desc: "Vi kartlegger dine ønsker, vippenes tilstand og anbefaler riktig stil og lengde." },
            { step: "02", title: "Forberedelse", desc: "Øynene renses og nedre vipper tapes forsiktig for å holde dem unna." },
            { step: "03", title: "Påsetting", desc: "Extensions festes én og én på dine naturlige vipper med medisinsk godkjent lim. Prosessen tar 1,5–2,5 timer." },
            { step: "04", title: "Tørking & avslutning", desc: "Limet tørker, du ser resultatet og vi gir deg tips for stell hjemme." },
          ].map((item) => (
            ...
            {faqs.map((faq, i) => (
              <div key={i}>
                <button
                  ...
                  className="flex w-full items-center justify-between py-5 text-left text-sm font-medium text-white/80 hover:text-white transition-colors"
                >
                  {faq.q}
                  <span className="ml-4 shrink-0 text-[color:var(--color-gold)] text-lg">
                    {openFaq === i ? "−" : "+"}
                  </span>
                </button>
                {openFaq === i && (
      ...
      <section className="mx-auto max-w-5xl px-8 py-16 text-center lg:px-16">
        ...
        <button
          type="button"
          onClick={() => setBookingOpen(true)}
          className="mt-8 inline-flex items-center gap-3 rounded-tl-lg rounded-bl-lg rounded-br-lg rounded-tr-none border border-[color:var(--color-gold)] px-8 py-4 text-sm tracking-[0.25em] text-[color:var(--color-gold)] transition-all duration-300 hover:bg-[color:var(--color-gold)]/10 hover:shadow-[0_0_20px_rgba(183,132,113,0.25)] active:scale-[0.98]"
        >
          Bestill time
        </button>
      </section>
